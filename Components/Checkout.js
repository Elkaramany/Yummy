...
function Checkout(props){
    ...
    const CardForm =async (data, price , deliver, address, method, fullDate, region, points)=>{
        try{
            ...
            const options = {
                ...
                axios.get(`https://arcane-ocean-58349.herokuapp.com/createStripePaymentIntent?money=${price}&&cur=${"rwf"}&&token=${token.tokenId}`).then(res =>{
                    if(res.data.outcome.seller_message === "Payment complete."){
                        props.makeOrder({data, price , deliver, address, method, fullDate, region, points});
    ...
    const navigateMeAway = () =>{
        ...
        points++;
        const {price, data, region} = props.navigation.state.params;
        ...
        if(cash){
            ...
            CardForm(data, price , deliver, address, method, fullDate, region, points);
        }else if(momo){
            if(mobile.length < 13){
                Alert.alert("Please enter a valid phone number")
            }else{
                method = "Customer Paid with MTN mobile money";
                MobilePay(data, price , deliver, address, method, fullDate, region);
            }
        }else{
